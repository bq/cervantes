...
WizardPagerCont::WizardPagerCont(QWidget* parent) : QWidget(parent){
        qDebug() << "--->" << Q_FUNC_INFO;
        setupUi(this);
...
WizardPagerCont::~WizardPagerCont()
{
    /// Do nothing
    qDebug() << "--->" << Q_FUNC_INFO;
}

void WizardPagerCont::nextPage(){
...
void WizardPagerCont::previousPage(){
        int pageNum = this->currentPageLbl->text().toInt() ;
...
void WizardPagerCont::setup(int totalPages){
        qDebug() << "--->" << Q_FUNC_INFO;
...
void WizardPagerCont::paintEvent (QPaintEvent *){
         QStyleOption opt;
         opt.init(this);
         QPainter p(this);
...
void WizardPagerCont::handleSwipe(int direction){
...
void WizardPagerCont::hidePages()
{
    currentPageLbl->hide();
    totalPagesLbl->hide();
    pageLbl->hide();

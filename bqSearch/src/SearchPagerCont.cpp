...
SearchPagerCont::SearchPagerCont(QWidget* parent) : QWidget(parent){
        qDebug() << "--->" << Q_FUNC_INFO;
        setupUi(this);
...
SearchPagerCont::~SearchPagerCont()
{
    /// Do nothing
    qDebug() << "--->" << Q_FUNC_INFO;
}

void SearchPagerCont::nextPage(){
...
void SearchPagerCont::previousPage(){
        int pageNum = this->currentPageLbl->text().toInt() ;
...
void SearchPagerCont::setup(int totalPages){
        qDebug() << "--->" << Q_FUNC_INFO;
...
void SearchPagerCont::handleSwipe(int direction){
...
void SearchPagerCont::paintEvent (QPaintEvent *){
         QStyleOption opt;
         opt.init(this);
         QPainter p(this);

...
QString Json::sanitizeString(QString str)
{
        str = bqUtils::specialCharactersFromUtf8(str);
...
QVariant Json::parseValue(const QString &json, int &index, bool &success)
{
        ...
        {
                case JsonTokenStringWhite:
                        //index--; // NOTE: Esta línea hace que cuando index == 0, se realice un acceso a json[-1]. ¿Tiene sentido esta linea? ¿Para qué vale?
...
int Json::nextToken(const QString &json, int &index)
{
        ...
        switch(c.toAscii())
        {
                ...
                default: return JsonTokenStringWhite;
        }
